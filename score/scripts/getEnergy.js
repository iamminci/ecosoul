...
function sum_total_energy_upper(data) {
  data_with_time_difference = data.map((elem, index, array) => {
    ...
    if (index > 0) {
      prevTime = data[index - 1].timestamp;
    }
    ...
    toReturn["total_energy_kWh_upper"] =
      Number(elem.total_energy_kW_upper) * difference_hours;
    return toReturn;
  });
...
async function request_data(start, end, code_name, minerID, outputOn) {
  ...
  if (returned_records_data.length == limit) {
    ...
    last_time =
      returned_records_data[returned_records_data.length - 1].timestamp;
    ...
    var new_data = await request_data(
      next_start_time,
      end,
      code_name,
      minerID,
...
async function get_total_energy_data(start, end, minerID, outputOn) {
  sealing_records_data = await request_data(
    start,
    end,
    "SealedModel",
    minerID,
  ...
  // Storage request
  storage_records_data = await request_data(
    start,
    end,
    "CapacityModel",
    minerID,
  ...
  if (sealing_records_data.length == limit) {
    throw new Error("Limit too short for sealing request");
  }
  if (storage_records_data.length == limit) {
    throw new Error("Limit too short for storage request");
  }
  ...
  lastBlockTime = new Date(
    storage_records_data[storage_records_data.length - 1].timestamp
  );
  ...
  if (!(requestEndTime - lastBlockTime) == 0) {
    newLastRecord = {
      ...
      capacity_GiB:
        storage_records_data[storage_records_data.length - 1].capacity_GiB,
  ...
  storage_with_time_difference = storage_records_data.map(
    (elem, index, array) => {
      prevTime = elem.timestamp;
      if (index > 0) {
        prevTime = storage_records_data[index - 1].timestamp;
      }
      prevTime_stamp = new Date(prevTime);
      currentTime_stamp = new Date(elem.timestamp);
      difference_ms = currentTime_stamp - prevTime_stamp;
  ...
  integrated_GiB_hr = storage_with_time_difference.reduce(
    (previousValue, elem) => {
  ...
  sealingEnergy_upper_MWh = (totalSealed_GiB * 5.6e-8 * 1024 ** 3) / 1e6;
  console.log("sealingEnergy_upper_MWh: ", sealingEnergy_upper_MWh);
  storage_upper_integrated_MWh =
    (integrated_GiB_hr * 8.1e-12 * 1024 ** 3) / 1e6;
  console.log("storage_upper_integrated_MWh: ", storage_upper_integrated_MWh);
  total_energy_upper_MWh_recalc =
    (sealingEnergy_upper_MWh + storage_upper_integrated_MWh) * PUE_upper;
  console.log("total_energy_upper_MWh_recalc: ", total_energy_upper_MWh_recalc);
  ...
  ending_time = new Date(
    storage_with_time_difference[
      storage_with_time_difference.length - 1
    ].timestamp
  ).getTime();
